...
async function buildWatchCommentaryV2({ imageInput, conversationMessages = [], trigger = "scene_change" }) {
  ...
  const prompt = [
    ...
    "优先聊玩法、任务、目标、界面变化、现在在做什么，尽量写成现场感描述，像搭子在旁边顺口接话。",
    "如果信息不完整，就猜玩法、猜目标、猜接下来会发生什么，但要像熟人陪看，不要像讲解员。",
    "可以偶尔自然带半句追问，但不能连续两轮都在问，也不要像盘问。",
  ...
  const result = await analyzeImageWithHistory({
    ...
    historyMessages,
    prompt: prompt.join("\n"),
    systemPrompt: "你是籽小刀。你在直播旁观位，只负责看图接话，不负责操作游戏。回复要自然、单句、少重复，多聊玩法和目标，少盯着 NPC 外观复读，不要使用“怕不是”句式。风格更像现场描述和顺口接话，不要写成评价腔，也不要连续发问。",
    maxTokens: 120,
    temperature: 0.9
  });
...
async function buildWatchUserReplyV2({ instruction, imageInput, conversationMessages = [] }) {
  ...
  const prompt = [
    ...
    "语气像熟人搭档，机灵一点，可以顺着他的话接，也可以轻轻带到玩法、任务、目标或画面里的新东西上，整体更像现场描述，不要写成评价腔。",
  ...
  const result = await analyzeImageWithHistory({
    ...
    historyMessages,
    prompt: prompt.join("\n"),
    systemPrompt: "你是籽小刀。你在直播旁观位，只负责看图接话，不负责操作游戏。优先像搭档一样自然回话，避免重复句式，不要使用“怕不是”句式。更像现场描述和顺口接话，不要写成评价腔，也不要一直问问题。",
    maxTokens: 120,
    temperature: 0.85
  });
