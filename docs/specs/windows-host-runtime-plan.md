...
- 潜行里的 `闷棍(3)` 和 `妙取(4)` 都已确认可以直接自动吃附近目标，不再要求先点人
...
- `recover_front_target_visibility` 这个 owner 仍保留在仓库里，但当前固定剧本的闷棍链和妙取链已经不再依赖它
...
2. 在 Windows 真机上确认最新叫卖收口是否稳定：`出摊 -> 改货/收摊 -> 正常 HUD`。
3. 继续补本地静态检查或 `node:test` 级别 smoke tests，减少后续文案池和 stage 配置回归。
4. 基于最新真机结论继续压缩潜行两段的无效前置步骤，避免又回到“先点人、先拉查看”的旧链路。
