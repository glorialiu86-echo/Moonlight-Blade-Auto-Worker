...
def locate_manual_talk_point(hwnd: int) -> dict:
...
def locate_manual_confirm_small_talk_point(hwnd: int) -> dict:
    # The confirm button after small talk is also fixed UI and should be clicked
    # directly from its manually marked relative coordinates.
    return build_screen_point_from_ratio(
        hwnd,
...
def locate_manual_chat_close_point(hwnd: int) -> dict:
    # The chat-page close button is fixed UI. Click it directly by relative
    # coordinates, but do not chain the next action immediately; the game needs
    # time to settle back into the clean town page after closing.
    return build_screen_point_from_ratio(hwnd, CHAT_CLOSE_BUTTON_CLIENT_RATIO_X, CHAT_CLOSE_BUTTON_CLIENT_RATIO_Y)
